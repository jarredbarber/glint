...
export async function getGitStatus(cwd: string): Promise<GitStatus> {
    try {
        ...
        if (remoteExists) {
            try { await gitExec(cwd, ['fetch', '--quiet']); } catch { }
        }

        let ahead = 0, behind = 0;
        if (branch && remoteExists) {
            try {
                const revList = await gitExec(cwd, ['rev-list', '--left-right', '--count', `origin/${branch}...HEAD`]);
...
export async function gitPull(cwd: string): Promise<GitPullResult> {
    try {
        ...
        const result = await gitExec(cwd, ['pull', '--ff-only']);
...
export async function gitPush(cwd: string, branch?: string): Promise<GitPushResult> {
    try {
        ...
        const result = await gitExec(cwd, ['push', 'origin', branchToPush]);
...
export async function gitSync(cwd: string, commitMessage?: string): Promise<GitSyncResult> {
    ...
    try {
        ...
        if (updatedStatus.behind > 0) {
            try {
                await gitExec(cwd, ['pull', '--ff-only']);
        ...
        if (finalStatus.ahead > 0) {
            try {
                await gitExec(cwd, ['push', 'origin', status.branch]);
